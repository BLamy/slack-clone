...
- Exact implementation commit: `31101554e960a1033cb6e50dd6b43b03a17ddf7e`.
- Exact cold command: `PROMOTE_EVIDENCE=1 E2_T07_IMPLEMENTATION_COMMIT=31101554e960a1033cb6e50dd6b43b03a17ddf7e TEST_RUN_ID=e2-t07-cold-final-v3 make verify-E2-T07`. The detached checkout was clean before install, initialized the pinned emulator, and recorded `make verify-E2-T07` as the entrypoint; all five required gates exited 0 with `skips: []`.
...
- Refusal evidence covers disabled config, suspended membership, non-member scope, unhealthy and incompatible providers, expired grants, over-budget use, and cross-workspace grants. Provider registry calls are counted for every refusal row: non-provider refusals make zero calls, while provider readiness failures make only the required registry checks; no provider side effect occurs. Five current-use revocation races reject stale authority while the historical snapshot remains replayable.
- Reconfiguration evidence changes config, provider manifest, membership, grant source, and workspace-input source; every new resolution has a new digest while the historical bytes remain unchanged. A scratch snapshot with its config source digest deleted is rejected by both replay and current-use verification with `INVOCATION_SNAPSHOT_DIGEST_MISMATCH`. Five source/digest mutants, including disabled digest verification, make the real verifier exit 1. Canary scanning covers canonical bytes, a resolver-read grant canary, refusal details, the promoted evidence set, and the final parent-written cold transcript; no provider token, credential, environment, or broker-session value is present.
