...
- Exact implementation commit: `4a69ba8238991613f51d73d6f8d26fdc87721d91`.
- Exact cold command: `PROMOTE_EVIDENCE=1 E2_T07_IMPLEMENTATION_COMMIT=4a69ba8238991613f51d73d6f8d26fdc87721d91 TEST_RUN_ID=e2-t07-cold-final-v2 make verify-E2-T07`. The detached checkout was clean before install, initialized the pinned emulator, and recorded `make verify-E2-T07` as the entrypoint; all five required gates exited 0 with `skips: []`.
...
- Reconfiguration evidence changes config, provider manifest, membership, grant source, and workspace-input source; every new resolution has a new digest while the historical bytes remain unchanged. Four source/digest mutants make the real verifier exit 1 through source-binding assertions. Canary scanning covers canonical bytes, a resolver-read grant canary, refusal details, and every promoted evidence file; no provider token, credential, environment, or broker-session value is present.
