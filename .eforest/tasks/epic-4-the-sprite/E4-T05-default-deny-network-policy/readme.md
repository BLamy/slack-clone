...
Every Sprite starts with default-deny egress and receives only the run-scoped destinations
needed for its configured broker and tool gateway. DNS resolution, redirects, IP changes,
private ranges, provider metadata endpoints, and inbound listeners are policed by one
server-owned policy whose decisions are recorded without payload or secret leakage.
...
- Versioned network-policy schema and compiler in `packages/sandbox` plus Fly enforcement.
...
- [ ] RFC1918, loopback, link-local, Unix host mounts, and cloud/Fly metadata endpoints
...
2. Attempt Fly/cloud metadata access and inbound listener exposure from the public and
