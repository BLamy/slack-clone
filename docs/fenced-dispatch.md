...
`sha256(canonicalJson(request))` before any provider append and runs authorization on
every attempt.
...
refused. Cross-stream receipt persistence is an idempotent saga: the target event is
