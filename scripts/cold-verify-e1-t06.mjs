  ...
  process.env.PROMOTE_EVIDENCE === "1"
    ...
    : path.resolve(
        process.env.TEST_ARTIFACT_DIR ??
          path.join(".artifacts", "e1-t06", runId),
      );
...
try {
  ...
  const artifactDirectory = process.env.TEST_ARTIFACT_DIR
    ...
    : path.join(checkout, ".artifacts", "e1-t06", runId);
  ...
  const verifier = await run("node", ["scripts/verify-e1-t06.mjs"], {
    name: "verify-E1-T06",
    cwd: checkout,
    env: {
      ...process.env,
      ...
      E1_T06_NETWORK_DISABLED: "1",
      TEST_ARTIFACT_DIR: artifactDirectory,
      TEST_RUN_ID: runId,
    },
  });
  transcript.commands.push({
    command: "node scripts/verify-e1-t06.mjs",
  ...
  if (process.env.PROMOTE_EVIDENCE === "1") {
    for (const filename of [
      ...
      "parser-corpus.json",
      "resolution-refusal-matrix.json",
      "dispatch-retry.json",
      "mention-replay-evidence.json",
      "sensitivity.json",
    ]) {
      await copyFile(
