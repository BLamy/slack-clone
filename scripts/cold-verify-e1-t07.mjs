  ...
  process.env.PROMOTE_EVIDENCE === "1"
    ...
    : path.resolve(
        process.env.TEST_ARTIFACT_DIR ??
          path.join(".artifacts", "e1-t07", runId),
      );
...
try {
  ...
  const artifactDirectory = process.env.TEST_ARTIFACT_DIR
    ...
    : path.join(checkout, ".artifacts", "e1-t07", runId);
  ...
  const verifier = await run("node", ["scripts/verify-e1-t07.mjs"], {
    name: "verify-E1-T07",
    cwd: checkout,
    env: {
      ...process.env,
  ...
  transcript.commands.push({
    command: "node scripts/verify-e1-t07.mjs",
    exitCode: verifier.code,
  });
  if (verifier.code !== 0) {
    throw new Error(`E1-T07 verifier failed: ${verifier.output}`);
  }
  ...
  if (process.env.PROMOTE_EVIDENCE === "1") {
    for (const filename of [
      ...
      "row-manifest-before-rebuild.json",
      "row-manifest-after-rebuild.json",
      "crash-recovery.json",
      "shadow-compare.json",
      "access-matrix.json",
