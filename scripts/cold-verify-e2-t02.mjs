...
const evidenceDirectory = path.join(taskDirectory, "evidence/e2-t02-final");
...
try {
  ...
  const artifactDirectory = process.env.TEST_ARTIFACT_DIR
    ...
    : path.join(checkout, ".artifacts", "e2-t02", runId);
  ...
  const verifier = await run("node", ["scripts/verify-e2-t02.mjs"], {
    name: "verify-E2-T02",
    cwd: checkout,
    env: {
      ...process.env,
      E2_T02_COLD_CLONE: "1",
      E2_T02_IMPLEMENTATION_COMMIT: implementationCommit,
      TEST_ARTIFACT_DIR: artifactDirectory,
      TEST_RUN_ID: runId,
    },
  });
  transcript.commands.push({
    command: "node scripts/verify-e2-t02.mjs",
    exitCode: verifier.code,
  });
  if (verifier.code !== 0) {
    throw new Error(`E2-T02 verifier failed: ${verifier.output}`);
  }
