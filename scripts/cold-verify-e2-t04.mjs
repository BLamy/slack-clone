...
if (!/^[0-9a-f]{40}$/u.test(implementationCommit)) {
  throw new Error(
    "E2-T04 cold verification requires an exact implementation commit",
  );
}
...
const evidenceDirectory = path.join(taskDirectory, "evidence/e2-t04-final");
...
try {
  ...
  const artifactDirectory = process.env.TEST_ARTIFACT_DIR
    ...
    : path.join(checkout, ".artifacts", "e2-t04", runId);
  ...
  const verifier = await run("node", ["scripts/verify-e2-t04.mjs"], {
    name: "verify-E2-T04",
    cwd: checkout,
    env: {
      ...process.env,
      E2_T04_COLD_CLONE: "1",
      E2_T04_IMPLEMENTATION_COMMIT: implementationCommit,
      TEST_ARTIFACT_DIR: artifactDirectory,
      TEST_RUN_ID: runId,
    },
  });
  transcript.commands.push({
    command: "node scripts/verify-e2-t04.mjs",
    exitCode: verifier.code,
  });
  if (verifier.code !== 0) {
    throw new Error(`E2-T04 verifier failed: ${verifier.output}`);
  }
