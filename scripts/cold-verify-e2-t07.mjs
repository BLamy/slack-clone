...
if (!/^[0-9a-f]{40}$/u.test(implementationCommit)) {
  throw new Error(
    "E2-T07 cold verification requires an exact implementation commit",
  );
}
...
const transcript = {
  ...
  entrypoint: {
    command:
      process.env.E2_T07_ENTRYPOINT ?? "node scripts/cold-verify-e2-t07.mjs",
    delegatedTo: "node scripts/cold-verify-e2-t07.mjs",
  },
...
try {
  ...
  const artifactDirectory = process.env.TEST_ARTIFACT_DIR
    ...
    : path.join(checkout, ".artifacts", "e2-t07", runId);
  ...
  const verifier = await run("node", ["scripts/verify-e2-t07.mjs"], {
    name: "verify-E2-T07",
    cwd: checkout,
    env: {
      ...process.env,
      E2_T07_COLD_CLONE: "1",
      E2_T07_IMPLEMENTATION_COMMIT: implementationCommit,
      TEST_ARTIFACT_DIR: artifactDirectory,
      TEST_RUN_ID: runId,
    },
  });
  transcript.commands.push({
    command: "node scripts/verify-e2-t07.mjs",
    exitCode: verifier.code,
  });
  if (verifier.code !== 0) {
    throw new Error(`E2-T07 verifier failed: ${verifier.output}`);
  }
