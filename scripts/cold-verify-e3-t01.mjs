...
if (!/^[0-9a-f]{40}$/u.test(implementationCommit)) {
  throw new Error(
    "E3-T01 cold verification requires an exact implementation commit",
  );
}
...
const artifactDirectory = path.join(checkout, ".artifacts", "e3-t01", runId);
const transcript = {
  schemaVersion: 1,
  runId,
  implementationCommit,
  entrypoint: {
    command: process.env.E3_T01_ENTRYPOINT ?? "make verify-E3-T01",
    delegatedTo: "node scripts/verify-e3-t01.mjs",
  },
...
try {
  ...
  execFileSync("node", ["scripts/verify-e3-t01.mjs"], {
    cwd: checkout,
    env: {
      ...process.env,
      E3_T01_COLD_CLONE: "1",
  ...
  if (worktreeAdded) {
    try {
      ...
      // Keep the original verifier failure visible to the caller.
    }
  }
