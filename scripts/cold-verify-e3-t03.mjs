...
if (!/^[0-9a-f]{40}$/u.test(implementationCommit)) {
  throw new Error(
    "E3-T03 cold verification requires an exact implementation commit",
  );
}
...
const reportDirectory = promoteEvidence
  ...
  : path.resolve(
      ...
      process.env.TEST_ARTIFACT_DIR ??
        path.join(".artifacts", "e3-t03-cold", runId),
    );
...
const artifactDirectory = path.join(checkout, ".artifacts", "e3-t03", runId);
const transcript = {
  schemaVersion: 1,
  runId,
  implementationCommit,
  entrypoint: {
    command: process.env.E3_T03_ENTRYPOINT ?? "make verify-E3-T03",
    delegatedTo: "node scripts/verify-e3-t03.mjs",
  },
...
const EVIDENCE_FILES = [
  ...
  "canary-scan.json",
  "capability-scopes.json",
  "lease-schedules.json",
  "queue-proof.json",
  "reducer-integration.json",
  "replay-digests.json",
  "sensitivity.json",
...
try {
  ...
  execFileSync("node", ["scripts/verify-e3-t03.mjs"], {
    cwd: checkout,
    env: {
      ...process.env,
      E3_T03_COLD_CLONE: "1",
  ...
  transcript.commands.push({
    command: "node scripts/verify-e3-t03.mjs",
    exitCode: 0,
  });
  ...
  const verifierEvidenceDirectory = promoteEvidence
    ? path.join(
        checkout,
        ".eforest/tasks/epic-3-the-dispatcher/E3-T03-durable-run-queue-and-leases/evidence/e3-t03-final",
      )
    : artifactDirectory;
  for (const filename of EVIDENCE_FILES) {
    await copyFile(
...
if (evidenceScan.leaked) {
  throw new Error(
    `E3-T03 evidence contains credential material: ${evidenceScan.files.find((file) => file.leaked)?.name ?? "unknown"}`,
  );
}
...
if (!canaryScan.postVerifierTranscriptChecked) {
  throw new Error("E3-T03 transcript was not included in the evidence scan");
}
...
if (finalEvidenceScan.leaked) {
  throw new Error(
    "E3-T03 evidence leaked credential material after scan update",
  );
}
