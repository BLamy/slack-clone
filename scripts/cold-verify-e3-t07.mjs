...
const reportDirectory = promoteEvidence
  ...
  : path.resolve(
      ...
      process.env.TEST_ARTIFACT_DIR ??
        path.join(".artifacts", "e3-t07-cold", runId),
    );
...
const evidenceFiles = [
  ...
  "reply-receipts.json",
  "sensitivity.json",
  "stale-authority.json",
  "verification-summary.json",
];

if (!/^[0-9a-f]{40}$/u.test(implementationCommit)) {
  throw new Error(
    "E3-T07 cold verification requires an exact implementation commit",
  );
}
...
const artifactDirectory = path.join(checkout, ".artifacts", "e3-t07", runId);
const transcript = {
  schemaVersion: 1,
  runId,
  implementationCommit,
  entrypoint: {
    command: process.env.E3_T07_ENTRYPOINT ?? "make verify-E3-T07",
    delegatedTo: "node scripts/verify-e3-t07.mjs",
  },
...
try {
  ...
  await runLogged("node", ["scripts/verify-e3-t07.mjs"], {
    cwd: checkout,
    displayCommand: "node scripts/verify-e3-t07.mjs",
    env: {
      ...process.env,
      E3_T07_COLD_CLONE: "1",
  ...
  const verifierEvidenceDirectory = promoteEvidence
    ? path.join(
        checkout,
        ".eforest/tasks/epic-3-the-dispatcher/E3-T07-provenance-bound-agent-replies/evidence/e3-t07-final",
      )
    : artifactDirectory;
...
async function finalizeEvidenceScan() {
  ...
  if (findings.length > 0) {
    throw new Error(
      `E3-T07 evidence leaked credential material: ${JSON.stringify(findings)}`,
    );
  }
  await writeFile(
