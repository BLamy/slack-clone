...
const reportDirectory = promoteEvidence
  ...
  : path.resolve(
      ...
      process.env.TEST_ARTIFACT_DIR ??
        path.join(".artifacts", "e3-t08-cold", runId),
    );
const workDirectory = path.join(taskDirectory, "work");
const evidenceFiles = [
  "attempt-timelines.json",
  "batching-recursion-outcomes.json",
  "canary-scan.json",
  "cancellation-revocation.json",
  "composite-replay-digests.json",
  "context-pack.json",
  "fault-schedules.json",
  "mention-invocation-snapshot.json",
  "process-resource-counts.json",
  "projection-rebuild.json",
  "queue-lease-manifest.json",
  "reply-receipt.json",
  "retry-usage-accounting.json",
  "sensitivity.json",
  "source-ref-manifest.json",
  "stream-dumps.json",
...
if (!/^[0-9a-f]{40}$/u.test(implementationCommit)) {
  throw new Error(
    "E3-T08 cold verification requires an exact implementation commit",
  );
}
...
const artifactDirectory = path.join(checkout, ".artifacts", "e3-t08", runId);
const transcript = {
  schemaVersion: 1,
  runId,
  implementationCommit,
  entrypoint: {
    command: process.env.E3_T08_ENTRYPOINT ?? "make verify-E3-T08",
    delegatedTo: "node scripts/verify-e3-t08.mjs",
  },
...
try {
  ...
  await runLogged("node", ["scripts/verify-e3-t08.mjs"], {
    cwd: checkout,
    displayCommand: "node scripts/verify-e3-t08.mjs",
    env: {
      ...process.env,
      E3_T08_COLD_CLONE: "1",
      E3_T08_IMPLEMENTATION_COMMIT: implementationCommit,
      E3_T08_SKIP_GATES: "0",
      E3_T08_SKIP_SENSITIVITY: "0",
  ...
  const verifierEvidenceDirectory = promoteEvidence
    ? path.join(
        checkout,
        ".eforest/tasks/epic-3-the-dispatcher/E3-T08-mention-to-scripted-agent-reply/evidence/e3-t08-final",
      )
    : artifactDirectory;
...
async function finalizeEvidenceScan() {
  ...
  if (findings.length > 0) {
    throw new Error(
      `E3-T08 evidence leaked credential material: ${JSON.stringify(findings)}`,
    );
  }
  await writeFile(
...
function redact(value) {
  return String(value)
    ...
    .replace(/\b(?:Bearer|Basic)\s+[A-Za-z0-9._~+/=-]{12,}/giu, "$1 [redacted]")
    .replace(
      /\b(?:sk|rk|pk)-[A-Za-z0-9][A-Za-z0-9_-]{15,}\b/gu,
      "[key-redacted]",
    )
    .replace(
