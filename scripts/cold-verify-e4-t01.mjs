...
const commit = String(
  process.env.E4_T01_IMPLEMENTATION_COMMIT ??
    execFileSync("git", ["rev-parse", "HEAD"], { cwd: root, encoding: "utf8" }),
).trim();
if (!/^[0-9a-f]{40}$/u.test(commit))
  throw new Error("E4-T01 requires an exact implementation commit");
...
try {
  ...
  execFileSync("node", ["scripts/verify-e4-t01.mjs"], {
    cwd: checkout,
    env: {
      ...process.env,
      E4_T01_IMPLEMENTATION_COMMIT: commit,
