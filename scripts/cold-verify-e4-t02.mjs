...
const commit = String(
  process.env.E4_T02_IMPLEMENTATION_COMMIT ??
    execFileSync("git", ["rev-parse", "HEAD"], { cwd: root, encoding: "utf8" }),
).trim();
if (!/^[0-9a-f]{40}$/u.test(commit))
  throw new Error("E4-T02 requires an exact implementation commit");
...
const task = path.join(
  root,
  ".eforest/tasks/epic-4-the-cloudflare-os/E4-T02-cloudflare-os-control-plane",
);
...
try {
  ...
  execFileSync("node", ["scripts/verify-e4-t02.mjs"], {
    cwd: checkout,
    env: {
      ...process.env,
      E4_T02_IMPLEMENTATION_COMMIT: commit,
