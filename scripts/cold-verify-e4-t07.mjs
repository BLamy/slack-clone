...
const commit = String(
  process.env.E4_T07_IMPLEMENTATION_COMMIT ??
...
if (!/^[0-9a-f]{40}$/u.test(commit))
  throw new Error("E4-T07 requires an exact implementation commit");
...
const task = path.join(
  root,
  ".eforest/tasks/epic-4-the-cloudflare-os/E4-T07-quotas-costs-and-orphan-gc",
);
...
const coldEnv = {
  ...process.env,
  GIT_CONFIG_COUNT: "1",
  GIT_CONFIG_KEY_0: "submodule.emulate.url",
  GIT_CONFIG_VALUE_0: "https://github.com/BLamy/emulate.git",
};
try {
  ...
  execFileSync("node", ["scripts/verify-e4-t07.mjs"], {
    cwd: checkout,
    env: {
      ...coldEnv,
      E4_T07_IMPLEMENTATION_COMMIT: commit,
  ...
  execFileSync("pnpm", ["format:check:e4-t07"], {
    cwd: checkout,
    stdio: "inherit",
  });
    ...
    JSON.stringify(
      {
        ...
        result: "PASS",
        runId,
        gates: ["format:check", "lint", "typecheck", "test:unit", "build"],
        replay:
