...
export type StreamStatus = Pick<
  StreamSnapshot,
  "stream" | "durableStreamsUrl" | "nextOffset"
> & {
  clients: number;
  streamDigest: string;
};
...
export async function fetchMembers() {
  ...
  return parseResponse<{ ok: true; members: DirectoryMember[] }>(response);
}
...
export function subscribeToRoom(
  ...
  handlers: {
    ...
    onReset: (reset: { nextOffset?: string; streamDigest?: string }) => void;
  ...
  const parseEvent = <T>(event: Event) =>
    JSON.parse((event as MessageEvent<string>).data) as T;

  events.addEventListener("open", handlers.onOpen);
  events.addEventListener("snapshot", (event) =>
    handlers.onSnapshot(parseEvent<StreamSnapshot>(event)),
  );
  events.addEventListener("message", (event) =>
    handlers.onMessage(parseEvent<MessageRecord>(event)),
  );
  events.addEventListener("status", (event) =>
    handlers.onStatus(parseEvent<StreamStatus>(event)),
  );
  events.addEventListener("reset", (event) =>
    handlers.onReset(
      parseEvent<{ nextOffset?: string; streamDigest?: string }>(event),
    ),
  );
  events.addEventListener("error", (event) => {
    const data = (event as MessageEvent<string>).data;
    handlers.onError(
      data ? parseEvent<{ message?: string }>(event).message : undefined,
    );
  });
