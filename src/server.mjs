...
function routeToStaticPath(url) {
  if (isChatRequest(url)) return "/index.html";
...
async function serveStatic(request, response, url) {
  ...
  const filePaths = [
    staticFilePath(clientDir, pathname),
    staticFilePath(publicDir, isChatRequest(url) ? "/app.html" : pathname),
  ].filter(Boolean);
  ...
  for (const filePath of filePaths) {
    try {
      ...
      // Try the legacy public root when the Vite client has not been built.
    }
  }
...
const server = createInboundHttpServer(async (request, response) => {
  ...
  try {
    ...
    const isPublicAsset =
      ...
      url.pathname === "/app.js" ||
      url.pathname === "/application-api.js";
