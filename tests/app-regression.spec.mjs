...
async function signIn(page, room, email, path = "/app") {
  ...
  await expect(page).toHaveURL(
    new RegExp(
      `${APP_BASE_URL.replace(/[.*+?^${}()|[\\]\\]/g, "\\\\$&")}${path}\\?room=${room}`,
    ),
  );
...
test("homepage leads through emulator login into the React chat", async ({
  page,
}) => {
...
test("unauthenticated app routes return to the emulator login", async ({
  page,
}) => {
...
test("Auth0 login errors preserve the form layout", async ({ page }) => {
...
test("owners can edit and delete messages while another user is forbidden", async ({
  browser,
}) => {
...
test("thread entry opens the AI response and full run logs", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("region", { name: "Thread view with agent run logs" }),
  ).toBeVisible();
  const response = page.getByRole("button", {
    name: "Open full run logs for Release concierge",
  });
  await response.press("Enter");
  await expect(
    page.getByRole("heading", { name: "Full run logs" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Run timeline" }),
  ).toBeVisible();
  await expect(page.getByRole("region", { name: "Raw log" })).toContainText(
    "secrets=[redacted]",
  );
  await page.getByRole("button", { name: "Close run logs" }).click();
  await expect(
    page.getByRole("heading", { name: "Inspect an agent response" }),
  ).toBeVisible();
  await page.getByRole("button", { name: "Close thread" }).click();
  await expect(
    page.getByRole("heading", { level: 1, name: `# ${room}` }),
  ).toBeVisible();
});
