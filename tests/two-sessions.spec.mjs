...
async function seedLegacyMessage(room, record) {
  const url = `http://127.0.0.1:4100/rooms/${encodeURIComponent(room)}/messages`;
  const headers = {
  ...
  const append = await fetch(url, { method: "POST", headers, body: JSON.stringify(record) });
...
test("homepage sends a user through the emulator login into the chat", async ({ page }) => {
  ...
  await expect(page.getByTestId("stream-digest")).toHaveText(/^sha256:[0-9a-f]{64}$/);
  const streamState = await page.evaluate(async () => {
    const response = await fetch("/api/rooms/demo/messages");
    return response.json();
  });
  await expect(page.getByTestId("stream-digest")).toHaveText(streamState.streamDigest);
});
...
test("a user can edit their message and the update persists across sessions", async ({ browser }) => {
  ...
  await ada.getByTestId("edit-message-input").fill("This cancelled draft must not persist");
  ...
  await ada.route(`**/api/rooms/${room}/messages/${messageId}`, async (route) => {
    await route.fulfill({
      status: 500,
  ...
  await originalMessage.getByRole("button", { name: "Edit message" }).click();
  await ada.getByTestId("edit-message-input").fill("This failed draft must not persist");
  await ada.getByRole("button", { name: "Save" }).click();
  await expect(ada.getByTestId("connection-state")).toHaveText("simulated edit failure");
  await expect(ada.getByTestId("edit-message-input")).toHaveValue("This failed draft must not persist");
  await ada.getByRole("button", { name: "Cancel" }).click();
  await expect(ada.getByText(original)).toBeVisible();
  ...
  await expect(ada.getByTestId("stream-digest")).toHaveText(streamState.streamDigest);

  await adaContext.close();
  await linusContext.close();
});

test("legacy display-name-only messages stay readable but cannot be edited", async ({ page }) => {
  const room = `legacy-${Date.now()}`;
  ...
  const denied = await page.evaluate(async ({ roomId, id }) => {
    const response = await fetch(`/api/rooms/${encodeURIComponent(roomId)}/messages/${encodeURIComponent(id)}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text: "Display-name collision must not grant ownership" }),
    });
    return { status: response.status, body: await response.json() };
